...
describe('ES6 New Grammar Test', () => {
  it('Two dimensional array to one dimensional array', () => {
    const input = [1, 2, 3, [4, 5]];
    const output = flatArray(input);
    expect(output).toEqual([1, 2, 3, 4, 5]);
  });

  it('One dimensional array to two dimensional array, double every item', () => {
    const input = [1, 2, 3, 4, 5];
    const output = aggregateArray(input);
    expect(output).toEqual([[2], [4], [6], [8], [10]]);
  });

  it("Get object's enumerable properties", () => {
    const input = {
      name: 'zhangsan',
      age: 18,
      sex: 'man',
  ...
  it('Count every item in array', () => {
    const input = [1, 1, 2, 2, 2, 1, 3, 4, 5, 4, 3, 1, 2];
